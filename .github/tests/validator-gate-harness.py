...
WHY THIS FILE EXISTS
  ...
  workflow against fakes for charly and gh, so the taxonomy is asserted in-repo on
  every review instead of being asserted in the PR body.
...
SCENARIOS ASSERTED END TO END (exit code + classification output + PR comment)
  ...
  for-attempt loop) - the R4 regression guard for the dropped retry band-aid - and
  the workflow pins a charly release WITH the taxonomy marker, never the old one.
...
ENVIRONMENT NOTES
  ...
  /tmp/review.log, /tmp/inconclusive-comment.md) literally. This harness removes
  those three files before and after every scenario; everything else it writes lives
  in a temp dir.
"""
...
RUNNER_PATHS = ["/tmp/review.txt", "/tmp/review.log", "/tmp/inconclusive-comment.md"]
